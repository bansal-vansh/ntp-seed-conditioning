...
from data.permutation import make_dataset, canonicalize, compute_permutation
...
eval_runs = 1
top_p=1.0
save_name = f"N{N}-H{H}-NT{num_train_samples}-E{epochs}-top_p{top_p}-{model_type}"
...
# %%
train_perms_set = set(tuple(p) for p in train_perms)
...
def evaluate_model(model, 
    ...
    for s in tokenized_samples:
        ...
        if pi is None:
            ...
            continue
        unique_perms.add(pi)
        if is_coherent(s, N=N, vocab_ids=VOCAB_IDs):
            num_coherent += 1
...
# %%
model_path = os.path.join("/datastor1/vansh/lang_sampling/results/", dataset_name, save_name, "best_models", f"HL{HL}")
...
def visualize_average_attention(model, tokenizer, num_samples=1000, max_length=20):
    ...
    with torch.no_grad():
        ...
        samples, outputs = generate_samples(
            model, 
            train_dataset, 
            tokenizer,
            decode_fn=decode_batch, 
        ...
        print(f"Generated {len(samples)} samples.")
        # print(f"Output IDs shape: {outputs["input_ids"].shape}")
        # 2. Perform a single forward pass on the generated IDs to get attentions.
        rich_outputs = model(input_ids=outputs["input_ids"].to(DEVICE), attention_mask=outputs["attention_mask"].to(DEVICE), output_attentions=True)
        attentions = rich_outputs.attentions # This is a tuple of tensors, one per layer

        print(f"Attentions shape: {len(attentions)} layers, each with shape {attentions[0].shape}")
    ...
    # --- Visualization ---
    readable_sequence = ['<BOS>'] + [f"HT {i+1}" for i in range(HL)] + [f'T{i+1}' for i in range(most_common_len - 1 - HL)] + ['<EOS>']
    ...
    ax.set_title(f"Average Attention Heatmap (Last Layer) over {len(maps_to_average)} samples with HL={HL}")
    fig.colorbar(im)
    fig.tight_layout()
    
    plt.savefig(f"{dataset_name}_attention_heatmap_HL{HL}.png")
